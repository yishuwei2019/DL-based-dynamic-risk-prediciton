...
def coxph_logparlk(event_time, event, hazard_ratio):
    ...
    for j in np.unique(event_time):
        ...
        sum_plus = sum(hazard_ratio[np.array([(x - j) > -TOL for x in event_time])])
        ...
        # print("sub1:", subtotal_1)
        # print("sub2:", subtotal_2)

        total = total + subtotal_1 - subtotal_2
    return Variable(torch.tensor(- total).type(torch.FloatTensor), requires_grad=True)
